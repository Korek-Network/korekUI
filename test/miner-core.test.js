...
import { MINER_PROTOCOL,createMinerAuthHeaders,formatKrk,miningStats,normalizeApiUrl,normalizeNodeUrl,rewardAddress,validateInnerHash,validateResources } from "../src/miner-core.js";
test("validates Planck endpoints",()=>{assert.equal(normalizeNodeUrl("http://127.0.0.1:9833/"),"http://127.0.0.1:9833");assert.equal(normalizeApiUrl("http://127.0.0.1:8365/"),"http://127.0.0.1:8365");assert.throws(()=>normalizeNodeUrl("file:///tmp/node"),/HTTP/)});
