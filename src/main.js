...
async function mineOnce(){
 ...
 try{
  ...
  const wait=Math.max(0,template.notBefore-Date.now());if(wait){emit({type:"paused",message:"Proof found — waiting for the permitted block time. CPU/GPU are idle; 100% is hashing intensity, not continuous utilization."});await new Promise(resolve=>setTimeout(resolve,wait))}if(!mining)return;
