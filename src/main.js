...
import { APP_VERSION,DEFAULT_API_URL,DEFAULT_NODE_URL,MINER_PROTOCOL,PLANCK_NETWORK,applyWorkerRate,miningStats,normalizeApiUrl,normalizeNodeUrl,rewardAddress,validateInnerHash,validateResources } from "./miner-core.js";
...
async function hardware(){
 ...
 const devices=(gpuInfo.gpuDevice||[]).map((item,index)=>{const name=item.deviceString||`GPU ${index+1} (${item.vendorId||"unknown"}:${item.deviceId||"unknown"})`;return{id:`gpu-${index}-${item.vendorId||0}-${item.deviceId||0}`,name,vendorId:item.vendorId||0,deviceId:item.deviceId||0,active:index===0,integrated:Number(item.vendorId)===32902,usable:!/Microsoft Basic Render Driver/i.test(name)}});
