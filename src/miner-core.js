...
export function rollingHashrate(samples,maxSamples=12){const valid=(samples||[]).filter(sample=>Number.isFinite(Number(sample?.hashes))&&Number(sample.hashes)>0&&Number.isFinite(Number(sample?.elapsedMs))&&Number(sample.elapsedMs)>0).slice(-maxSamples),hashes=valid.reduce((sum,sample)=>sum+Number(sample.hashes),0),elapsedMs=valid.reduce((sum,sample)=>sum+Number(sample.elapsedMs),0);return elapsedMs?Math.round(hashes/(elapsedMs/1000)):0}
export function validateResources(input,hardware){const maxThreads=Math.max(1,Number(hardware?.threads||1)),requestedThreads=Number(input?.cpuThreads),cpuThreads=Math.max(0,Math.min(maxThreads,Number.isFinite(requestedThreads)?Math.trunc(requestedThreads):1)),known=new Set((hardware?.gpu||[]).filter(device=>device.usable!==false).map(device=>device.id)),gpuIntensity=Math.max(0,Math.min(100,Math.trunc(Number(input?.gpuIntensity)||0))),gpuIds=gpuIntensity>0?[...new Set((input?.gpuIds||[]).filter(id=>known.has(id)))].slice(0,1):[];if(cpuThreads===0&&gpuIds.length===0)throw new Error("Set CPU above 0 or GPU above 0 before mining");return{cpuThreads,gpuIds,gpuIntensity}}
